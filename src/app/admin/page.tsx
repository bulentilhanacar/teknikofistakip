...
const NewUserForm = ({ onUserAdded }: { onUserAdded: () => void }) => {
    ...
    const handleAddUser = async () => {
        ...
        try {
            ...
            await addDoc(collection(firestore, 'users'), { email, role });
            toast({ title: "Kullanıcı Eklendi", description: `${email} sisteme ${role} rolüyle eklendi.` });
            setEmail('');
            setRole('user');
            onUserAdded();
        } catch (error) {
...
export default function AdminPage() {
    ...
    const { data: users, isLoading: usersLoading, error } = useCollection<AppUser>(usersQuery);
